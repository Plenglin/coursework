...
public class UDGraph
{
    ...
    public UDGraph length2Paths() {
        ...
        boolean[][] mat = multiplyAdjMatrices(adjMatrix, adjMatrix);
        newGraph.applyAdjMatrix(mat);
        return newGraph;
    }

    public void applyAdjMatrix(boolean[][] mat) {
        for (int i=0; i < vertices; i++) {
            for (int j=0; j < vertices; j++) {
                if (mat[i][j]) {
                    addEdge(i, j);
    ...
    /**
     * Multiply 2 square boolean matrices together.
     * @param m1
     * @param m2
     * @return
     */
    public static boolean[][] multiplyAdjMatrices(boolean[][] m1, boolean[][] m2) {
        int vertices = m1.length;
        boolean[][] out = new boolean[vertices][vertices];
        for (int i=0; i < vertices; i++) {
            for (int j=0; j < vertices; j++) {
                boolean sum = false;
    ...
    public UDGraph paths(int length) {
        boolean[][] out = identityMatrix(vertices);
        boolean[][] mul = adjMatrix;
        while (length > 0) {  // exponentiation by squaring
            if ((length & 1) != 0) {
                out = multiplyAdjMatrices(out, mul);
            }
            length >>= 1;
            mul = multiplyAdjMatrices(mul, mul);
        }
        UDGraph newGraph = new UDGraph(vertices);
        newGraph.applyAdjMatrix(out);
        return newGraph;
    }

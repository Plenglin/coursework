...
int SudokuContext::run() {
	while (true) {
		...
		std::string path = constants.dataPath + "sudoku"
				+ std::to_string(diff - 1) + ".txt";
...
void SudokuContext::play(SudokuBoard board) {
	do {
		...
		if (1 <= x && x <= 9 && 1 <= y && y <= 9 && 0 <= val && val <= 9) {
			...
			if (!result) {
				std::cout << "Error: (" << x << ", " << y << ") is immutable."
						<< std::endl;
			}
		} else {
			std::cout
					<< "Error: numbers out of range. x and y must be in [1, 9], set must be in [1, 9], or 0 to clear cell."
					<< std::endl;
		}
...
int SudokuContext::askForDifficulty() {
	...
	do {
		...
		for (int i = 1; i <= 5; i++) {
			cout << i << ". " << constants.difficultyLocalizations[i - 1]
					<< endl;
		}
		cout << "Difficulty: ";
		...
		cout << "You selected " << diff << " (" << diffName << ")"
				<< ". Confirm? (y/n) ";
